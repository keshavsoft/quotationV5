...
$MinPath    = Join-Path $DataDir ($InputFile -replace '\.json$', '_min.json')
...
$minMB = node $MinifyScript $Utf8Path $MinPath
Write-Host "      Done  →  $minMB MB" -ForegroundColor Green

# ── Step 3: Replace original & clean up ─────────────────────────────────────
Write-Host "[3/3] Replacing original file ..." -ForegroundColor Yellow
Remove-Item $InputPath
Rename-Item $MinPath $InputPath
...
$finalMB = [math]::Round((Get-Item $InputPath).Length / 1MB, 2)
